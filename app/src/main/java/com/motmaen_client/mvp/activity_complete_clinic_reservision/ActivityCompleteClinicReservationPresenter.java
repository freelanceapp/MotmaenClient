...
public class ActivityCompleteClinicReservationPresenter {
    ...
    public void updateresrevision(UserModel userModel,  SingleReservisionTimeModel.Detials singletimemodel, String date,  int reservid) {
        if(userModel!=null) {
            view.onLoad();
            Api.getService(Tags.base_url)
                    .updatereservision(reservid+ "",  date, singletimemodel.getFrom(),singletimemodel.getFrom_hour_type())
                    .enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            view.onFinishload();
